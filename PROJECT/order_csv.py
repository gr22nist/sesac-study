...
class ORD_DataGNRT:
    ...
    def __init__(self, num):
        ...
        with open('./data/store.csv', 'r', encoding='utf-8') as file:
            reader = csv.DictReader(file)
            for d in reader:
                self.strid.append(d)    
        
        with open('./data/user.csv', 'r', encoding='utf-8') as file:
            reader = csv.DictReader(file)
            next(reader)
            for d in reader:
                self.usid.append(d)
...
#     with open('./data/order.csv', 'w', encoding='utf-8') as csv_file:
